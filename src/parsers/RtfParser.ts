...
export const parseRtf = async (buffer: Buffer, config: FullOfficeParserConfig): Promise<OfficeParserAST> => {
    ...
    let currentParagraphRawChunks: string[] = [];

    // State for text run construction
    let currentRunTextChunks: string[] = [];
            ...
            if (headingLevel === undefined && currentParagraphChildren.length > 0) {
                ...
                if (firstChild.type === 'text' && firstChild.formatting?.bold) {
                    ...
                    if (size >= 14 && currentParagraphText.length < 300) {
                        if (size >= 22) headingLevel = 1;
                        else if (size >= 18) headingLevel = 2;
                        else if (size >= 16) headingLevel = 3;
                        else headingLevel = 4;
                    }
                }
            }
    ...
    const extractGroupText = (group: RtfGroup): string => {
        let text = '';
    ...
    const traverse = (node: RtfNode, formatting: TextFormatting, depth: number = 0) => {
        if (node.type === 'group') {
            ...
            if (isPict) {
                ...
                for (const child of node.content) {
                    if (child.type === 'control') currentParagraphRawChunks.push(serializeRtfControl(child));
                    else if (child.type === 'text') currentParagraphRawChunks.push(serializeRtfText(child));
                    else if (child.type === 'group') {
                        ...
                        const serializeGroupContent = (g: RtfGroup) => {
                            for (const c of g.content) {
                                if (c.type === 'control') currentParagraphRawChunks.push(serializeRtfControl(c));
                                else if (c.type === 'text') currentParagraphRawChunks.push(serializeRtfText(c));
...
function findRtfGroup(group: RtfGroup, destination: string): RtfGroup | null {
    for (const node of group.content) {
        if (node.type === 'group') {
            if (node.destination === destination) return node;
