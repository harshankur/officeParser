 ...
 * @property {number} contentNodes
 * @property {{ total: number; withListId: number; withItemIndex: number; ordered: number; unordered: number }} lists
 * @property {{ total: number; rows: number; cells: number }} tables
 * @property {{ total: number; byLevel: Record<number, number> }} headings
 * @property {number} images
 * @property {{ total: number; internal: number; external: number }} links
 * @property {{ total: number; footnotes: number; endnotes: number }} notes
 * @property {{ total: number; withOCR: number; charts: number }} attachments
 * @property {Record<string, number>} formatting
 * @property {{ hasStyleMap: boolean; styleMapSize: number; hasTitle: boolean; hasAuthor: boolean }} metadata
 */
...
function getFilePath(ext, isBaseline = false) {
    if (isBaseline) {
 ...
 */
function countTabLinkNodes(ast) {
    let count = 0;
...
function getOdpVirtualDiscrepancies(ast) {
    let emptyNodes = 0;
    let emptyLists = 0;
    let tablePadding = 0;
    let mergedCells = 0;
...
function applyOdpParityAdjustments(actual, ast, expected) {
 ...
 */
function comparePdfParity(groupName, expected, actual) {
    const results = [];
 ...
 */
function compareRtfParity(category, expected, actual) {
    const results = [];
...
function validateTextExtraction(category, fileType, ast, metrics) {
    const results = [];
...
function compareTextBaseline(category, fileType, extractedText) {
    const results = [];
    // Load baseline text
...
async function testGroupParity(group, groupName) {
    const results = [];
...
async function testConfigs(ext) {
    const filePath = getFilePath(ext);
    if (!fs.existsSync(filePath)) {
        return [];
    }
...
function generateReport(allResults, logger) {
    const width = 130;
...
async function runSingleFileTest(ext) {
...
async function generateParsedOutputs() {
