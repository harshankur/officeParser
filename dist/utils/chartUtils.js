 ...
 * @param xmlBuffer Chart XML buffer
 */
const extractChartexChartData = (xmlBuffer) => {
    const xml = xmlBuffer.toString("utf8");
    const dom = (0, xmlUtils_1.parseXmlString)(xml);
    // Find plotArea
    const plotArea = (0, xmlUtils_1.getElementsByTagName)(dom, "cx:plotArea")[0];
    if (!plotArea) {
    ...
    const plotAreaRegion = (0, xmlUtils_1.getElementsByTagName)(plotArea, "cx:plotAreaRegion")[0];
    if (!plotAreaRegion) {
        return {
            title: undefined,
            xAxisTitle: undefined,
    ...
    const seriesNodes = (0, xmlUtils_1.getElementsByTagName)(plotAreaRegion, "cx:series");
    let chartType = undefined;
    ...
    // Extract chartData
    const chartData = (0, xmlUtils_1.getElementsByTagName)(dom, "cx:chartData")[0];
    if (!chartData) {
        return {
            title: undefined,
            chartType: undefined,
    ...
    const dataMap = {};
    const dataElements = (0, xmlUtils_1.getElementsByTagName)(chartData, "cx:data");
    ...
    for (const serNode of seriesNodes) {
        const dataIdNode = (0, xmlUtils_1.getElementsByTagName)(serNode, "cx:dataId")[0];
        ...
        const strDim = (0, xmlUtils_1.getElementsByTagName)(dataEl, "cx:strDim")[0];
        if (strDim && strDim.getAttribute("type") === 'cat') {
            const lvlNodes = (0, xmlUtils_1.getElementsByTagName)(strDim, "cx:lvl");
            ...
            if (levelCount > 0) {
                ...
                for (let idx = 0; idx < ptCount; idx++) {
                    ...
                    for (const lvl of lvlNodes) {
                        const pts = (0, xmlUtils_1.getElementsByTagName)(lvl, "cx:pt");
                        const pt = Array.from(pts).find(p => parseInt(p.getAttribute("idx") || '-1', 10) === idx);
                        if (pt) {
        ...
        if (categories.length === 0) {
            const numDimCat = Array.from((0, xmlUtils_1.getElementsByTagName)(dataEl, "cx:numDim")).find(dim => dim.getAttribute("type") === 'catVal');
            if (numDimCat) {
                const lvl = (0, xmlUtils_1.getElementsByTagName)(numDimCat, "cx:lvl")[0];
                if (lvl) {
                    const pts = (0, xmlUtils_1.getElementsByTagName)(lvl, "cx:pt");
                    for (const pt of pts) {
                        const ptText = pt.textContent || '';
                        if (ptText)
                            categories.push(ptText);
                    }
                }
            }
        }
        ...
        const values = [];
        const numDimVal = Array.from((0, xmlUtils_1.getElementsByTagName)(dataEl, "cx:numDim")).find(dim => dim.getAttribute("type") === 'size' || dim.getAttribute("type") === 'val');
        if (numDimVal) {
            const lvl = (0, xmlUtils_1.getElementsByTagName)(numDimVal, "cx:lvl")[0];
            if (lvl) {
                const pts = (0, xmlUtils_1.getElementsByTagName)(lvl, "cx:pt");
                for (const pt of pts) {
                    const ptText = pt.textContent || '';
        ...
        let name = undefined;
        const txNode = (0, xmlUtils_1.getElementsByTagName)(serNode, "c:tx")[0];
        if (txNode) {
    ...
    let xAxisTitle = undefined;
    let yAxisTitle = undefined;
    const catAxes = (0, xmlUtils_1.getElementsByTagName)(dom.documentElement, "c:catAx");
    if (catAxes.length > 0)
        xAxisTitle = extractOpenXmlRichText(catAxes[0], "c:title");
    const valAxes = (0, xmlUtils_1.getElementsByTagName)(dom.documentElement, "c:valAx");
    if (valAxes.length > 0)
        yAxisTitle = extractOpenXmlRichText(valAxes[0], "c:title");
    // Build rawTexts
    if (title)
