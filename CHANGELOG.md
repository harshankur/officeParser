...
- **`GeneratorConfig.metadataOverrides`** sets the metadata embedded in generated output without mutating the parsed AST. Named fields mirror `OfficeMetadata` (`title`, `author`, `description`, `subject`, `keywords`, `lastModifiedBy`, `created`, `modified`, `language`) plus a `custom` bucket for caller-defined key/value pairs, kept separate so a typo like `titel` is a compile error rather than a silently-created custom entry. Merged per field over `ast.metadata`, so overriding one field leaves the rest intact, and applied uniformly across every generator that embeds metadata (HTML `<meta>`, EPUB OPF, Markdown frontmatter, RTF `\info`, and the `renderMetadata` header in text/CSV).
  - `modified` is written by EPUB as both the required `dcterms:modified` property and each zip entry's mtime. When unset it now falls back to the source document's own `ast.metadata.modified` before the current time, and a date outside the 1980-2099 range a ZIP timestamp can represent is clamped rather than throwing.
...
- **Generating the same AST to EPUB twice produced different bytes.** Two independent wall-clock sources fed the archive: `dcterms:modified` in the OPF, and the mtime `fflate` stamps onto every zip entry when none is supplied. The second is easy to miss, because DOS zip timestamps have two-second granularity, so back-to-back generation looks stable and only a longer gap reveals it. Both now derive from a single resolved instant (`metadataOverrides.modified`, else `ast.metadata.modified`, else now), and a date outside zip's representable 1980-2099 window clamps instead of throwing.
